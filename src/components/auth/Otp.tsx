...
const router = useRouter()
    ...
    return (
        <div className=" bg-white shadow-md rounded-[13px] overflow-hidden">
            <h2 className="text-2xl text-center text-white bg-primary py-[18px] font-medium text-[24px]  mb-6">Enter your Otp</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-6">
                <div>
                    <Label htmlFor="otp" className="text-lg">OTP</Label>
                    <Input className="mt-2 h-12 px-2 text-lg text-[#a5a6a8]" id="otp" type="number" placeholder="Enter your otp" {...register("otp")} />
                    {errors.otp && <p className="text-red-500 text-sm ">{errors.otp.message}</p>}
                </div>


                <Button type="submit" className="w-full bg-primary py-6">Enter OTP</Button>
            </form>
